...
$results = null;
$title = "Input";
$path_proses = "modules/create_karyawan.php";
if (isset($_GET['id_karyawan'])) {
    $curl = new Curl();
    $id_karyawan = $_GET['id_karyawan'];

    // Query API /karyawan 
    $curl->get('http://localhost:5000/karyawan/' . $id_karyawan);

    $results = (!empty($curl->response->data)) ? $curl->response->data[0] : array();
    $curl->close();
    $title = "Edit";
    $path_proses = "modules/edit_karyawan.php";
}
...
<head>
    ...
    <title>Form Karyawan - Aplikasi Absensi</title>
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    <h1 class="mt-4">Form <?= $title ?> Data Karyawan</h1>
                        ...
                        <div class="card-body">
                            <form action="<?= $path_proses ?>" method="post">
                                ...
                                if (!empty($results)) {
                                    ...
                                    // echo "</pre>";
                                ?>
                                    <input type="hidden" name="id_karyawan" value="<?= $results->id ?>" />
                                    <div class="mb-3">
                                        <label for="inputUsername">Nama Karyawan</label>
